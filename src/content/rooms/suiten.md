---
name: Suiten
variants: Comfort · Superior
tagline: Exklusiver Wohnkomfort
size: ab 48 m²
capacity: 1 bis 2 Personen
highlight: false
sort_order: 3
seo_title: Suiten, Zimmer & Suiten, feelMOOR
seo_description: Suiten Comfort und Superior im feelMOOR Gesundresort Bad Wurzach. Ab 48 m², separater Wohnbereich, Therme-Zugang inklusive. Jetzt buchen.
hero_image: /images/zimmer/Superior-Suite/90-a-0091-IRZ_9892-ingorack.com.jpg
preview_image: /images/zimmer/Superior-Suite/86-a-0087-IRZ_9888-ingorack.com.jpg
gallery:
  - image: /images/zimmer/Superior-Suite/87-a-0088-IRZ_9889-ingorack.com.jpg
    caption: Suite Wohnbereich
  - image: /images/zimmer/Superior-Suite/88-a-0089-IRZ_9890-ingorack.com.jpg
    caption: Schlafbereich
  - image: /images/zimmer/Superior-Suite/92-a-0093-IRZ_9894-ingorack.com.jpg
    caption: Badezimmer
  - image: /images/zimmer/Superior-Suite/94-a-0095-IRZ_9896-ingorack.com.jpg
    caption: Ausblick
intro_heading: Die Suite
intro_subheading: "Raum für Zweisamkeit, auf höchstem Niveau"
specs:
  - label: Größe
    value: ab 48 m²
  - label: Belegung
    value: 1 bis 2 Personen
  - label: Kategorien
    value: Comfort · Superior
  - label: Wohnbereich
    value: Separater Wohnbereich mit Sofa
  - label: Badezimmer
    value: Freistehende Badewanne & Regendusche
  - label: Aussicht
    value: Besonderer Ausblick (je nach Suite)
feature_groups:
  - heading: Komfort
    items:
      - Separater Wohnbereich
      - Luxuriöses Boxspringbett
      - Premium-Kissenmenü
      - Kuschelroben & Hausschuhe
      - Sitzgruppe & Schreibtisch
  - heading: Badezimmer
    items:
      - Freistehende Badewanne
      - Regendusche
      - feelMOOR Moorwasser-Kosmetikserie Premium
      - Doppelwaschbecken
      - Fußbodenheizung
  - heading: Ausstattung
    items:
      - Flatscreen TV (65")
      - WLAN kostenlos
      - Nespresso Maschine & Wasserkocher
      - Minibar Premium
      - Zimmersafe & Bügelbrett
  - heading: Therme & Service
    items:
      - Therme-Zugang inbegriffen
      - Frühstücksbuffet inklusive
      - Tägliche Zimmerreinigung
      - Turndown Service
      - 24h-Concierge
---

Unsere Suiten in den Kategorien Comfort und Superior bieten exklusiven Wohnkomfort auf hohem Niveau. Ein separater Wohnbereich, edle Materialien und besondere Details machen Ihren Aufenthalt zu einem unvergesslichen Erlebnis.

Die großzügige Raumaufteilung schafft Platz für echte Erholung, ob Sie gemeinsam den Abend auf dem Sofa verbringen oder in der freistehenden Badewanne den Stress des Alltags hinter sich lassen möchten.

Alle Suiten sind mit unserer exklusiven Moorwasser-Kosmetikserie ausgestattet. Der Therme-Zugang ist im Übernachtungspreis inbegriffen.
